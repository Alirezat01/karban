...
type SalarySettings = {
  ...
  insuranceEmployeeRate?: number; insuranceEmployerRate?: number; annualTaxFree?: number;
};
...
export default function CalculatorPage({ type, title, description }: Props) {
  ...
  const results = useMemo(() => {
    ...
    const family = isMarried ? 5000000 : 0; // 500,000 Tomans
    const child = childrenCount * (cfg.baseSalaryDaily * 3);
    const overtime = (base / 220) * 1.4 * overtimeHours;
    const absence = (base / 220) * absenceHours;
    const severanceMonthly = (base / 12) * years; // سنوات ماهانه=(base/12)×years

    // subject items for insurance: base, food, housing. (Family, child, severance usually exempt).
    ...
    let tax = taxable > 0 ? taxable * 0.1 : 0; // simplified 10%
