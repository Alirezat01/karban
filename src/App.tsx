...
export default function App() {
  ...
  else if (segments[0] === 'دانشنامه') { content = segments.length === 1 ? <ContentPage kind="knowledge" title="راهنمای قانون کار و تأمین اجتماعی، به زبان ساده اما مستند" description="مقالات کاربردی حقوق کار، بیمه و مالیات به زبان ساده." eyebrow="دانشنامه" /> : <ArticlePage title={`مقاله ${segments[1]}`} category="دانشنامه کاربان" />; title = segments.length === 1 ? 'دانشنامه حقوقی و مالیاتی کسب‌وکار | کاربان' : `مقاله ${segments[1]}`; description = 'مقالات کاربردی حقوق کار، بیمه و مالیات به زبان ساده.'; breadcrumb = ['دانشنامه', ...(segments.length > 1 ? [`مقاله ${segments[1]}`] : [])]; }
  else if (segments[0] === 'قراردادها') { content = segments.length === 1 ? <ContentPage kind="contracts" title="بانک قراردادهای کاربان — دانلود نمونه قرارداد آماده" description="بیش از ۶۰ قرارداد تخصصی به تفکیک صنف؛ متن کامل و PDF." eyebrow="قراردادها" /> : <ArticlePage title={`جزئیات قرارداد ${segments.slice(1).join(' ')}`} category="قراردادهای کاربان" contractId={segments[1]} />; title = segments.length === 1 ? 'بانک نمونه قرارداد | دانلود قرارداد کار، همکاری، پیمانکاری | کاربان' : `قرارداد ${segments.slice(1).join(' ')}`; description = 'بیش از ۶۰ قرارداد تخصصی به تفکیک صنف؛ متن کامل و PDF.'; breadcrumb = ['قراردادها', ...(segments.length > 1 ? [segments.slice(1).join(' ')] : [])]; }
  else if (segments[0] === 'خدمات') { content = <ServicesPage />; title = 'مشاوره مالی، مالیاتی و روابط کار | خدمات تخصصی کاربان'; description = 'مشاوره و قرارداد اختصاصی برای هر صنف؛ از پزشکان تا فروشگاه آنلاین.'; breadcrumb = ['خدمات']; }
  else if (segments[0] === 'ابزارهای-هوش-مصنوعی') { const calc = calcMap[segments[1]]; content = segments.length === 1 ? <ContentPage kind="tools" title="ابزارهای هوش مصنوعی کاربان" description="ماشین‌حساب‌های دقیق حقوق، سنوات، اضافه‌کاری و مالیات مطابق مقررات ۱۴۵." eyebrow="ابزارهای هوش مصنوعی" /> : calc ? <CalculatorPage type={calc.type} title={calc.title} description={calc.desc} /> : segments[1] === 'ساخت-قرارداد' || segments[1] === 'تست-سلامت' ? <ContentPage kind="simple" title={segments[1] === 'ساخت-قرارداد' ? 'ساخت قرارداد هوشمند' : 'تست سلامت کسب‌وکار'} description="با چند پاسخ کوتاه، مسیر مناسب تصمیم‌گیری را پیدا کنید." eyebrow="ابزار هوشمند" /> : <NotFound />; title = calc?.title || (segments.length === 1 ? 'ابزارهای هوش مصنوعی کاربان | محاسبه حقوق، سنوات، بازنشستگی ۱۴۰۵' : 'ابزار هوشمند'); description = calc?.desc || 'ماشین‌حساب‌های دقیق حقوق، سنوات، اضافه‌کاری و مالیات مطابق مقررات ۱۴۵.'; breadcrumb = ['ابزارهای هوش مصنوعی', ...(segments.length > 1 ? [title] : [])]; }
