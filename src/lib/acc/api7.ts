...
export async function accountCard(
  ...
  const { data: chart } = await supabase
    ...
    .or(`business_id.is.null,business_id.eq.${businessId}`);
  const tree = new Map<string, { title: string | null; kind: string; nature: string | null }>();
  for (const c of chart || []) tree.set(c.code as string, { title: (c as { title?: string | null }).title ?? null, kind: c.kind as string, nature: c.nature as string | null });
...
export async function closeFiscalYearV2(businessId: string, jyear: number): Promise<YearCloseResultV2> {
  ...
  /* مسیر اتمیک (بند ۱۶) — سند اختتامیه + بستن سال + قفل ۱۲ ماه در یک تراکنش.
     نوشتن مستقیم acc_periods/acc_fiscal_years با M150000 بسته شده است. */
  const { data: rpcId, error: rpcErr } = await supabase.rpc('acc_close_fiscal_year', {
    p_business: businessId,
    p_jyear: jyear,
    p_date: range.to,
    p_description: `سند اختتامیه سال مالی ${jyear}`,
    p_lines: closingLines.map((l) => ({
      account_code: l.account_code,
      account_title: l.account_title || l.account_code,
      debit: num(l.debit), credit: num(l.credit),
      detail_id: l.detail_id ?? null,
  ...
  const closingId = rpcId as string;

  /* شمارهٔ سند اختتامیه از دفتر خوانده می‌شود (شماره‌گذاری فقط در موتور) */
  const { data: closingRow } = await supabase
    .from('acc_journal').select('entry_no').eq('id', closingId).maybeSingle();

  return { jyear, closingEntryNo: Number(closingRow?.entry_no ?? 0), revenueTotal, expenseTotal, netProfit, closedAccounts: revMap.size + expMap.size };
}
...
export async function openNextYear(businessId: string, jyear: number): Promise<{ entryNo: number; movedAccounts: number }> {
  ...
  /* مسیر اتمیک (بند ۱۶) — سند افتتاحیه + ثبت سال بعد در یک تراکنش */
  const { data: rpcId, error: rpcErr } = await supabase.rpc('acc_open_next_year', {
    p_business: businessId,
    p_jyear: jyear + 1,
    p_date: jalaliYearRange(jyear + 1).from,
    p_description: `سند افتتاحیه سال مالی ${jyear + 1}`,
    p_lines: openLines.map((l) => ({
      account_code: l.account_code,
      account_title: l.account_title || l.account_code,
      debit: num(l.debit), credit: num(l.credit),
      detail_id: l.detail_id ?? null,
  ...
  const { data: openingRow } = await supabase
    .from('acc_journal').select('entry_no').eq('id', rpcId as string).maybeSingle();

  return { entryNo: Number(openingRow?.entry_no ?? 0), movedAccounts: openLines.length };
}
...
export async function saveOpeningEntry(
  ...
  /* پیوند اتمیک افتتاحیه به سال مالی — از مسیر RPC (نوشتن مستقیم از M150000 بسته است) */
  const { error: linkErr } = await supabase.rpc('acc_fiscal_link_entry', {
    p_business: businessId, p_jyear: jyear, p_entry: id,
  });
